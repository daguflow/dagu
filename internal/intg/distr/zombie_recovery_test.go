...
`, indentYAMLBlock(rollingReplacementLogCommand(beforeReplacement, afterReplacement, releaseFile), 6),
	...
	waitForOutput := func(marker string) {
		...
		}, distrTestTimeout(runningTimeout), 100*time.Millisecond, "step output should reach a coordinator: %s", marker)
	}
	waitForOutput(beforeReplacement)
	...
	require.NoError(t, os.WriteFile(releaseFile, []byte("ok"), 0o600))
	waitForOutput(afterReplacement)
